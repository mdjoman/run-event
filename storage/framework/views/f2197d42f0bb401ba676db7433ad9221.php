            ...
            <tbody class="divide-y divide-slate-100">
                <?php $__empty_1 = true; $__currentLoopData = $events; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $event): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500"><?php echo e($event->category); ?></p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-5 py-4 font-semibold text-slate-800">BDT <?php echo e(number_format($event->fee)); ?></td>
                        <td class="px-5 py-4">
                            <?php
                                $colors = [
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick='openViewModal(<?php echo json_encode($event, 15, 512) ?>)'
                                        class="w-8 h-8 flex items-center justify-center rounded-lg bg-blue-50 text-brandBlue hover:bg-blue-100 transition"
                                        title="View">
                                    <i class="fa-solid fa-eye text-xs"></i>
                                </button>
                                <button onclick='openEventModal(<?php echo json_encode($event, 15, 512) ?>)'
...
<div id="eventModal" class="fixed inset-0 bg-black/50 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form id="eventForm" method="POST" action="<?php echo e(route('admin.events.store')); ?>" class="p-6 space-y-4">
            <?php echo csrf_field(); ?>
...
<script>
    function openEventModal(event = null) {
        ...
        const base = "<?php echo e(route('admin.events.store')); ?>";

        if (event) {
            title.textContent = 'Edit Event';
            form.action = "<?php echo e(url('admin/events')); ?>/" + event.id;
            method.value = 'PUT';
